<?php require_once  './MVC/views/shared/header.php' ?>
<div class="container-fluid  main">
    <div class="d-flex m-auto justify-content-center sign-up">
        <div class="p-3 bg-white rounded nav-tabs " style="max-width: 500px; ">
            <!-- Ảnh title -->
            <div class="brand nav-tabs mb-5 ">
                <div class="image mb-3 ">
                    <img src="./public/img/Spotify_Logo_CMYK_Black.png " alt=" ">
                </div>
                <p>
                    <strong style="font-size: larger;">Sign up for free to listen</strong>
                </p>
            </div>
            <!-- Form đăng ký -->
            <form class="pb-4 nav-tabs " action="auth/authSignup" method="post">
                <div class="mb-3 ">
                    <label for="exampleInputEmail1 " class="form-label "><strong>Email address or phone
                            number</strong></label>
                    <input name="email" type="email" class="form-control " id="email" aria-describedby="emailHelp "
                        placeholder="Email or phone" pattern=".+@gmail\.com" required>
                </div>
                <div class="mb-3 ">
                    <label for="exampleInputPassword1 " class="form-label"><strong>Password</strong></label>
                    <input name="password" type="password " class="form-control " id="exampleInputPassword1 "
                        placeholder="Password" required>
                </div>

                <div class="mb-3 ">
                    <label for="exampleInputPassword1 " class="form-label"><strong>Username</strong></label>
                    <input name="userName" type="text" class="form-control " id="exampleInputPassword1 "
                        placeholder="Profile name" required>
                    <div id="emailHelp " class="form-text ">You'll see this name in your profile</div>
                </div>
                <!-- Ngày tháng năm sinh -->
                <label for="" class="form-label"><strong>Day of birth</strong> </label>
                <div class="mb-3 row">
                    <div class="form-group col-3">
                        <label for="day">Day</label>
                        <select name="day" id="day" class="form-control" placeholder="DD">
                        </select>
                    </div>
                    <div class="form-group col-6">
                        <label for="month">Month</label>
                        <select name="month" id="month" class="form-control" placeholder="MM">
                        </select>
                    </div>
                    <div class="form-group col-3">
                        <label for="year">Year</label>
                        <select name="year" id="year" class="form-control" placeholder="YYYY">
                        </select>
                    </div>
                </div>
                <!-- Giới tính -->
                <label for="" class="form-label"><strong>Gender</strong> </label>
                <div class="form-group">
                    <div class="form-check-inline">
                        <input name="gender" type="radio" class="form-check-input" id="radio1" value="male" checked>Male
                        <label class="form-check-label" for="radio1"></label>
                    </div>
                    <div class="form-check-inline">
                        <input name='gender' type="radio" class="form-check-input" id="radio2" value="female">Female
                        <label class="form-check-label" for="radio2"></label>
                    </div>

                </div>
                <div class="alert alert-danger d-none mt-2">Error!!!</div>

                <div class="mt-5 ">
                    <button type="submit" class="btn w-100 d-block spotify-btn p-3 text-white ">
                        <strong>SIGN UP</strong>
                    </button>
                </div>
            </form>
            <!-- Những cách đăng nhập khác  -->
            <div class="login_ways mt-4 ">
                <p class="text-center "><strong>Or sign up with</strong></p>
                <div class="row d-flex justify-content-center ">
                    <div class="login_way col-2 ">
                        <a href="https://wwww.facebook.com " target="_blank ">
                            <img src="data:image/png;base64,"
                                alt=" ">
                        </a>
                    </div>
                    <div class="login_way col-2 ">
                        <a href="https://www.google.com " target="_blank ">
                            <img src="https://www.freepnglogos.com/uploads/google-logo-png/google-logo-png-suite-everything-you-need-know-about-google-newest-0.png "
                                alt=" ">
                        </a>
                    </div>
                    <div class="login_way col-2 ">
                        <a href="https://www.github.com " target="_blank ">
                            <img src="https://www.freepnglogos.com/uploads/512x512-logo-png/512x512-logo-github-icon-35.png "
                                alt=" ">
                        </a>
                    </div>
                </div>
            </div>

            <!-- Hoặc đăng ký tài khoản mới  -->


        </div>
    </div>
</div>

<?php require_once './MVC/views/shared/footer.php' ?>